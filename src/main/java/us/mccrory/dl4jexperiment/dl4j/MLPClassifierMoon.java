 ...
 *
 */
public class MLPClassifierMoon extends AbstractClassifier {
	...
	public static void main(String[] args) throws Exception {
		...
		final String filenameTrain = new ClassPathResource("/moon_data_train.csv").getFile().getPath();
		final String filenameTest = new ClassPathResource("/moon_data_eval.csv").getFile().getPath();
		...
		MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder().seed(seed).updater(new Nesterovs(learningRate, 0.9)).list()
				...
				.layer(1, new OutputLayer.Builder(LossFunction.NEGATIVELOGLIKELIHOOD).weightInit(WeightInit.XAVIER).activation(Activation.SOFTMAX).nIn(numHiddenNodes)
		...
		rr.initialize(new FileSplit(new ClassPathResource("/moon_data_train.csv").getFile()));
		rr.reset();
